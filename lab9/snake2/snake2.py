...
def random_food_position(snake_list):
    while True:
        ...
        if [foodx, foody] not in snake_list:
            return foodx, foody
#function to randomly generating food 
def generate_food():
    food_types = ['normal', 'special', 'rare']
...
def draw_food(foodx, foody, color):
...
def gameLoop():
    ...
    food_color, food_timeout = generate_food()
    food_timer = pygame.time.get_ticks() + food_timeout
 
    while not game_over:
        #Timer
        current_time = pygame.time.get_ticks()
        if current_time >= food_timer:
            foodx, foody = random_food_position(snake_List)
            food_color, food_timeout = generate_food()
            food_timer = current_time + food_timeout
